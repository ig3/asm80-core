...
Parser.prototype = {
  parse: function (expr) {
    ...
    while (this.pos < this.expression.length) {
      if (this.isNumber()) {
        ...
        const token = new Token(TNUMBER, 0, 0, this.tokennumber);
        ...
        if (expected & NULLARY_CALL) {
          const token = new Token(TNUMBER, 0, 0, []);
          tokenstack.push(token);
        ...
        const consttoken = new Token(TNUMBER, 0, 0, this.tokennumber);
        ...
        const vartoken = new Token(TVAR, this.tokenindex, 0, 0);
  ...
  evaluate: function (expr, variables) {
    ...
    const value = this.parse(expr).evaluate(variables);
    return value;
  },
  ...
  isSign: function () {
    const code = this.expression.charCodeAt(this.pos - 1);
    if (code === 45 || code === 43) {
      // -
      return true;
    }

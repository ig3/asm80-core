 ...
 */
  ...
  parseOpcode: function (s, vars, Parser) {
    ...
    const R16 = function (reg) {
      const n = ['BC', 'DE', 'HL', 'SP'].indexOf(reg.toUpperCase());
      return n;
    };
    const R16IX = function (reg) {
      const n = ['BC', 'DE', 'IX', 'SP'].indexOf(reg.toUpperCase());
      return n;
    };
    const R16IY = function (reg) {
      const n = ['BC', 'DE', 'IY', 'SP'].indexOf(reg.toUpperCase());
      return n;
    };
    const R16A = function (reg) {
      const n = ['BC', 'DE', 'HL', 'AF'].indexOf(reg.toUpperCase());
      return n;
    };
    const COND = function (reg) {
      const n = ['NZ', 'Z', 'NC', 'C', 'PO', 'PE', 'P', 'M'].indexOf(reg.toUpperCase());
      return n;
    };
    ...
    let ax = Z80.set[s.opcode];
    const ax2 = Z80.set2[s.opcode];
    let op = -1; let bytes = 1; let lens = [];
    let prefix = null; let disp = null;
    let reg, param8, mode, idx;
      ...
      if (!s.params || s.params.length === 0) {
        ...
        if (s.opcode === 'LD') {
          ...
          if (s.bytes === 0) {
            reg1 = R16(par1);
            reg2 = R16(par2);
            const link1 = LINK(par1);
            const link2 = LINK(par2);

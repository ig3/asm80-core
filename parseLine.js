...
*/
  if (t) {
    ...
    if (pp && pp[1].length) {
      ...
      const n = ppc.match(/([0-9]+)\s*DUP\s*\((.*)\)/i);
      if (n) {
        const dup = parseInt(n[1]);
        let nln = '';
        for (let i = 0; i < dup; i++) {
          nln += n[2] + ',';
        }
  ...
  if (!line.label && !opts.stopFlag) {
    ...
    const s2 = JSON.parse(JSON.stringify(line));
    s2.addr = null;
    s2.bytes = 0;
    ...
    // hotfix
    // console.log(line)
    if (line.params[0].indexOf(':=') === 0) { line.params[0] = '.SET' + line.params[0].substr(2); }

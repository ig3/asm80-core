...
const doParse = async (data) => {
  try {
    const lines = await Parser.parse(data, { assembler: I8080 });
    return lines;
  } catch (e) {
    console.log(e);
    return e;
...
const testParse = async (data, showError = false) => {
  try {
    const lines = await Parser.parse(data, { assembler: I8080, readFile: dummyreadFile });
...
QUnit.test('Macro params OK', async assert => {
  const o = await Parser.parse(`.macro test, src, dst
    db %%src
    dw %%dst
   .endm
...
QUnit.test('Macro params too few', async assert => {
  asyncThrows(assert, () => {
    const o = testParse(`.macro test, src, dst
    db %%src
    dw %%dst
   .endm
